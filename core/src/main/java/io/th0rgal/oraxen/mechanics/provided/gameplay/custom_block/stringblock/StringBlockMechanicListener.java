...
public class StringBlockMechanicListener implements Listener {
    ...
    public void onPlacingString(PlayerInteractEvent event) {
        ...
        if (StringBlockMechanicFactory.get().disableVanillaString) {
    ...
    public void onPrePlacingCustomBlock(final PlayerInteractEvent event) {
        ...
        if (item != null && item.getType().isBlock() && !StringBlockMechanicFactory.get().isNotImplementedIn(itemID)) {
            for (BlockFace face : BlockFace.values()) {
        ...
        if (mechanic.hasRandomPlace()) {
            ...
            customVariation = StringBlockMechanicFactory.get().getMechanic(randomBlock).getCustomVariation();
        }
